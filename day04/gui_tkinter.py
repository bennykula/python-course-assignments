"""Tkinter GUI BMI calculator wrapper."""

import tkinter as tk
from tkinter import ttk

try:
    from day03.bmi_service import evaluate_bmi
except ModuleNotFoundError:
    from bmi_service import evaluate_bmi


def calculate_and_display(weight_var, height_var, unit_var, result_var):
    """Read user input from widgets, calculate BMI, and display results."""
    try:
        weight = float(weight_var.get())
        height = float(height_var.get())
        unit_system = unit_var.get()
        bmi, category = evaluate_bmi(weight, height, unit_system)
        result_var.set(f"BMI: {bmi} | Category: {category}")
    except ValueError as error:
        result_var.set(f"Error: {error}")


def update_labels(unit_var, weight_label, height_label):
    """Update labels based on selected unit system."""
    if unit_var.get() == "metric":
        weight_label.config(text="Weight (kg):")
        height_label.config(text="Height (m or cm):")
    else:
        weight_label.config(text="Weight (lbs):")
        height_label.config(text="Height (inches):")


def main():
    """Build and run the Tkinter BMI calculator UI."""
    root = tk.Tk()
    root.title("BMI Calculator")
    root.resizable(False, False)

    frame = ttk.Frame(root, padding=16)
    frame.grid(row=0, column=0, sticky="nsew")

    weight_var = tk.StringVar()
    height_var = tk.StringVar()
    unit_var = tk.StringVar(value="metric")
    result_var = tk.StringVar(value="Enter values and click Calculate")

    # Unit system selection
    ttk.Label(frame, text="Unit System:").grid(row=0, column=0, sticky="w", padx=(0, 8), pady=(0, 8))
    unit_frame = ttk.Frame(frame)
    unit_frame.grid(row=0, column=1, sticky="w", pady=(0, 8))
    
    ttk.Radiobutton(
        unit_frame,
        text="Metric",
        variable=unit_var,
        value="metric",
        command=lambda: update_labels(unit_var, weight_label, height_label),
    ).pack(side="left", padx=(0, 16))
    
    ttk.Radiobutton(
        unit_frame,
        text="Imperial",
        variable=unit_var,
        value="imperial",
        command=lambda: update_labels(unit_var, weight_label, height_label),
    ).pack(side="left")

    weight_label = ttk.Label(frame, text="Weight (kg):")
    weight_label.grid(row=1, column=0, sticky="w", padx=(0, 8), pady=(0, 8))
    ttk.Entry(frame, textvariable=weight_var, width=20).grid(row=1, column=1, pady=(0, 8))

    height_label = ttk.Label(frame, text="Height (m or cm):")
    height_label.grid(row=2, column=0, sticky="w", padx=(0, 8), pady=(0, 8))
    ttk.Entry(frame, textvariable=height_var, width=20).grid(row=2, column=1, pady=(0, 8))

    ttk.Button(
        frame,
        text="Calculate",
        command=lambda: calculate_and_display(weight_var, height_var, unit_var, result_var),
    ).grid(row=3, column=0, columnspan=2, pady=(4, 8))

    ttk.Label(frame, textvariable=result_var).grid(row=4, column=0, columnspan=2, sticky="w")

    root.mainloop()


if __name__ == "__main__":
    main()
