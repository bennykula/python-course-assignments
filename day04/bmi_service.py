...
CM_TO_M_CONVERSION_THRESHOLD = 50  # If height > 50 with metric units, assume cm and convert to meters
...
def calculate_bmi(weight, height, unit_system="metric"):
    ...
    Args:
        ...
        height: Height value (meters for metric, inches for imperial).
                If height > 100 with metric units, assumes cm and converts to meters.
        unit_system: "metric" (default) or "imperial"

    Returns:
        float: BMI value rounded to 2 decimal places
    """
    # Auto-convert cm to meters if height looks like cm (metric only)
...
def get_bmi_category(bmi):
    """Map a BMI value to its category (same for both unit systems)."""
    if bmi < 18.5:
        return "Underweight"
    if bmi < 25:
